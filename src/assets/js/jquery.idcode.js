...
(function ($) {
	...
	function _setCodeStyle(eid, codeType, codeLength) {
		...
		htmlCode += '" href="#" onblur="return false" onfocus="return false" oncontextmenu="return false" >' + _setStyle(codeObj) + '</div>' + '<span id="ehong-code-tip-ck" class="ehong-code-val-tip" >' + settings.codeTip + '</span>';
		$(eid).html(htmlCode);
...
//初始化数据
export let domButton = function () {
	$.idcode.setCode();
	$("#btns").click(function () {
		isBoolean = $.idcode.validateCode();
	});
}

//点击切换验证码
export const domclick = (dom, bol) => {
	$("#" + dom + "").on('click', () => {
		$.idcode.setCode()
	})
}
